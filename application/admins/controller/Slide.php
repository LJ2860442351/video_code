...
class Slide extends BaseAdmin
{
	public function index()
	{
		...
		$data['data'] = $this->db->table('ppt')->where($where)->order('id desc')->pages($data['pageSize']);
	...
	public function add()
	{
		...
		$data['item'] = $this->db->table('ppt')->where(array('id'=>$id))->item();
		$this->assign('data',$data);
		return $this->fetch();	
	}
	...
	public function upload_img()
	{
		...
		if(!in_array($ext, array('ppt','pptx','png','jpeg','jpg'))){
	...
	public function save(){
		...
		if($data['title'] == ''){
			exit(json_encode(array('code'=>1,'msg'=>'ppt名称不能为空')));
		}
		if($data['url'] == ''){
			exit(json_encode(array('code'=>1,'msg'=>'ppt不能为空')));
		}
		if($id){
			$this->db->table('ppt')->where(array('id'=>$id))->update($data);
		}else{
			$data['add_time'] = time();
			$this->db->table('ppt')->insert($data);
		}
	...
	public function delete(){
		$id = (int)input('post.id');
		$this->db->table('ppt')->where(array('id'=>$id))->delete();
