...
class Document extends BaseAdmin
{
	public function index()
	{
		...
		$data['data'] = $this->db->table('document')->where($where)->order('id desc')->pages($data['pageSize']);
	...
	public function add()
	{
		...
		$data['item'] = $this->db->table('document')->where(array('id'=>$id))->item();
		$this->assign('data',$data);
		return $this->fetch();	}
	...
	public function save(){
		...
		if($data['title'] == ''){
			exit(json_encode(array('code'=>1,'msg'=>'文档名称不能为空')));
		}
		if($data['url'] == ''){
			exit(json_encode(array('code'=>1,'msg'=>'地址不能为空')));
		}
		if($id){
			$this->db->table('document')->where(array('id'=>$id))->update($data);
		}else{
			$data['add_time'] = time();
	...
	public function delete(){
		$id = (int)input('post.id');
		$this->db->table('document')->where(array('id'=>$id))->delete();
